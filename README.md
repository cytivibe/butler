...
- `addtask` - create a task (50 chars max)
  ...
  - Blocks if a non-archived task with the same name exists at the same level (case-insensitive, across parallel/sequential); use `--force` to archive the existing one
...
- `settask "task:pos"` - update name, status, description, verification, tags, or blockers
  - Flags: `--status`, `--desc`, `--verify`, `--verify-status`, `--deadline`, `--recur`, `--tag`, `--force` (can combine multiple in one call)
  - Renaming blocks if a non-archived task with the new name exists at the same level (case-insensitive); use `--force` to archive the conflicting one
  ...
  - `butler settask "task:1" --verify "criteria"` - set verification criteria (auto-sets status to pending)
  - `butler settask "task:1" --verify none` - clear verification criteria and status
  - `butler settask "task:1" --verify-status passed` - mark verification as passed (allows completion)
  - `butler settask "task:1" --verify-status pending` - revert verification to pending (blocks completion)
...
- `gettask` - view tasks (requires `--all`, a task ref, `--tag`, `--nottag`, or `--status`)
  - Flags: `--all`, `--status "STATUS"`, `--tag TAG1 TAG2`, `--nottag TAG1 TAG2`, `--depth N`, `--sort recent`, `--details`
  ...
  - `butler gettask --tag URGENT BACKEND` - tasks with both tags (AND)
  - `butler gettask --tag NONE` - all tasks with no tags
  - `butler gettask --nottag ARCHIVED` - exclude tasks tagged ARCHIVED
  - `butler gettask --tag URGENT --nottag DONE` - urgent tasks not tagged DONE
  ...
  - `butler gettask --all --details` - includes created date, desc, verify, verify status, and applicable rules
...
- When a recurring task is `completed` or `not_started` and its next occurrence is due, it auto-transitions to `active` and resets verification status to `pending` if criteria exist
...
- A task cannot be `completed` if it has verification criteria with status `pending` (use `--verify-status passed` first)
