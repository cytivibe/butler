...
3. **Minimize calls.** Use flags (status, tag, depth, sort, details) to get exactly what is needed per call. Do not fetch the same information twice.
...
4. Tag strategically: tag at the highest relevant level — children inherit parent tags. Do not redundantly tag children with tags their parent already carries. Reuse existing tags. Create new tags with `addtag` only when needed. Tags: uppercase alphanumeric, 10 chars max, `NONE` is reserved. Common categories: project name, technology, domain, priority. Apply with `settask` using `tags` (replaces existing tags on that task). Tags should form a coherent taxonomy, not an ad-hoc collection.
